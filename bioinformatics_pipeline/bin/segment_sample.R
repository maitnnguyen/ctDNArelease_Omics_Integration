...
source(file.path(bin_dir, "R/EM.R"))

# Check mandatory inputs
if(is.null(opt$input)) stop("BAM file (--input) must be provided")
if(is.null(opt$centromere)) stop("Centromere file (--centromere) must be provided")
if(is.null(opt$gcmap)) stop("Interval's GC content annotation file (--gcmap) must be provided")
if(is.null(opt$PoN)) stop("Panel of Normal file (--PoN) must be provided")
...
for (i in 1:numSamples) {
  id <- 'sample' #SampleID#files[i,'SampleID']
  ...
  if (!is.null(normal_copy)){
    if (gender$gender != 'female'){ #use tumour # use normal if given
...
#write.table(loglik, '/loglik.csv', col.names=T, row.names=F,quote=F,sep='\t')
...
#outFile <- "params.txt"
outputParametersToFile(hmmResults.cor, file = paste0(outDir,"/params.txt"))
